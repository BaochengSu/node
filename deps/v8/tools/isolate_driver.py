...
def load_ninja_recursively(build_dir, ninja_path, build_steps):
  ...
  try:
    ...
    print('Failed to open %s' % ninja_path, file=sys.stderr)
    raise
  ...
  for rel_path in subninja:
    try:
      ...
      print('... as referenced by %s' % ninja_path, file=sys.stderr)
      raise
  return total
...
def raw_build_to_deps(item):
  ...
  return list(filter(using_blacklist, item.split(' ')[1:]))
...
def post_process_deps(build_dir, dependencies):
  ...
  return list(filter(is_exe, list(map(filter_item, dependencies))))
