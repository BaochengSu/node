...
def install(paths, dst): list(map(lambda path: try_copy(path, dst), paths))
def uninstall(paths, dst): list(map(lambda path: try_remove(path, dst), paths))
...
def npm_files(action):
  ...
  for dirname, subdirs, basenames in os.walk('deps/npm', topdown=True):
    subdirs[:] = list(filter('test'.__ne__, subdirs)) # skip test suites
...
def subdir_files(path, dest, action):
  ...
  for subdir, files in list(ret.items()):
    action(files, subdir + '/')
